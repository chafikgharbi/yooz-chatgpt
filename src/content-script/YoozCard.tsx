...
function YoozCard({
  ...
  const updatePromptHistory = (prompt: string) => {
    const newPromptHistory = [prompt, ...promptHistory.filter((p) => p !== prompt)].slice(0, 5)
  ...
  const replaceSelection = (text) => {
    if (isSelection && isFormInput) {
      ...
      ;(targetInput as HTMLFormElement).value =
        allText.substring(0, inputSelectionStart) +
        text +
        allText.substring(inputSelectionEnd, allText.length)
    } else {
  ...
  return (
    <Draggable disabled={!draggable} handle=".yooz-card-header" defaultPosition={position}>
      <div
        className="yooz-card"
        ref={ref}
        style={
          position
            ? {
                marginTop:
                  position.y - hegiht - topMargin > 0 ? -hegiht - topMargin : -position.y + 10,
                marginLeft:
                  position.x + width > window.innerWidth
                    ? -(position.x + width - window.innerWidth + 10)
                    : 0,
              }
            : {}
        }
      >
        ...
        <div className="yooz-card-content" ref={contentRef}>
          ...
          {(isSelection || isMessage) && (
            <>
              {/* // TODO make an option to show context (for edition purpose) */}
              {contextPreview ? (
                <div
                  className="yooz-context-preview"
                  dangerouslySetInnerHTML={{ __html: contextPreview.outerHTML }}
                />
              ) : (
                <div className="yooz-card-row">
                  <label className="yooz-card-label">
                    {_t(isSelection ? 'selectionLabel' : 'messageLabel')}
                  </label>
                  <textarea
                    rows={10}
                    id="yooz-context"
          ...
          <div className="yooz-card-row">
            <form autoComplete="on">
              <label className="yooz-card-label yooz-prompt-label">
                ...
                <div
                  className={`yooz-history-button ${showHistory ? 'yooz-active' : ''}`}
              ...
              {retry > 0 &&
                (() => {
                  if (isBraveBrowser()) {
                    ...
                    return (
                      <span className="italic block mt-2 text-xs">{_t('loginMessageDesc')}</span>
                    )
                  }
                })()}
            </a>
